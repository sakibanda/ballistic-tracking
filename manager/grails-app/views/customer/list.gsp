	...
	<table class="table table-bordered">
		<thead>
			<tr>
				...
				<g:sortableColumn property="apiKey" title="${message(code: 'customer.apiKey.label', default: 'Api Key')}" />
			
				<g:sortableColumn property="createOn" title="${message(code: 'customer.createOn.label', default: 'Create On')}" />
			
				<g:sortableColumn property="email" title="${message(code: 'customer.email.label', default: 'Email')}" />
			
				<g:sortableColumn property="keyEnabled" title="${message(code: 'customer.keyEnabled.label', default: 'Key Enabled')}" />
		...
		<g:each in="${customerInstanceList}" status="i" var="customerInstance">
			<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
				...
				<td>${fieldValue(bean: customerInstance, field: "apiKey")}</td>
			
				<td><g:formatDate date="${customerInstance.createOn}" /></td>
			
				<td>${fieldValue(bean: customerInstance, field: "email")}</td>
			
				<td>${fieldValue(bean: customerInstance, field: "keyEnabled")}</td>
