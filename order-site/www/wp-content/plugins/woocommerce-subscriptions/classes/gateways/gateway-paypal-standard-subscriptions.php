...
class WC_PayPal_Standard_Subscriptions {
	...
	public static function add_subscription_form_fields(){
		...
		foreach ( $woocommerce->payment_gateways->payment_gateways as $key => $gateway ) {
			...
			$woocommerce->payment_gateways->payment_gateways[$key]->form_fields['receiver_email']['description'] .= ' </p><p class="description">' . __( 'It is <strong>strongly recommended you do not change the Receiver Email address</strong> if you have active subscriptions with PayPal. Doing so can break existing subscriptions.', WC_Subscriptions::$text_domain );
	...
	public static function check_subscription_payment( $subscription_key ) {
		...
		if ( 'active' === $subscription['status'] ) {
			...
			if ( $last_payment_plus_two_days < gmdate( 'U' ) ) {
				...
				//$order->add_order_note( __( 'Subscription payment failure triggered as no recurring payment has been processed by PayPal.', WC_Subscriptions::$text_domain ) );
